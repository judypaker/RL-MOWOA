...
%% Reinforcement Learning enhanced Multi-Objective Whale Optimization Algorithm (RL-MOWOA)
% 
% Input parameters:
...
% ishow - Display interval
%
% Output:
% f - Optimal solution set
...
K = D + M;
Whale_pos = Pop(:, 1:K+2);  % Pop contains K+2 columns after sorting (including rank and crowding distance)
Whale_pos_ad = zeros(SearchAgents_no, K);

%% Metrics and Parameter Recording
...
while eval_count < Max_evals
    %% Get Current State and Select Action (Parameter Combination)
    ...
    %% Execute MOWOA Iteration Using Parameters Selected by RL
    for i = 1:SearchAgents_no
        ...
        % Adaptive scaling factor selected by RL agent
        SF = params(1); 
        
        % Select from first front weighted by crowding distance to enhance diversity
        ...
        if ~isempty(front_indices)
            ...
            % If no first front, select a random individual
        ...
        % Parasite strategy (adjusted using RL parameters)
        j = floor(rand() * SearchAgents_no) + 1;
        while j == i
            j = floor(rand() * SearchAgents_no) + 1;
        end
    ...
    %% Population update and non-domination sorting (silent)
